select a.*,b.max_arr --- cari krediti olmayan
from (
...
ml.zarr_days,ml.loan_category,la.outs_balance,ml.partner
...
where ml.status ='S006'
and not exists (
select uuid,status from me_loan mll where mll.status = 'S005' and ml.partner = mll.partner
)
and not exists (
